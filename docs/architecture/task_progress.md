...
1. [ ] A11.6 SSA IR 到 Machine IR 过渡层，具体边界后续在 `ssa_mem2reg_design.md` 中继续收敛。
2. [ ] A12 基础 IR 优化 Pass。
3. [ ] A13 RISC-V 寄存器分配优化。

推进原则：

- 先补基础设施，再启用行为变化。
- 每个阶段都要保留 `-koopa`、`-riscv` 或 smoke 级别的最小验证路径。
- 不把 Machine IR、寄存器分配、完整 SSA lowering 混在同一个任务里完成。
...
- 2026-05-22: 为 A11 新增 `docs/architecture/ssa_mem2reg_design.md`。A11 的 SSA 合流表示确定为 basic block arguments 与控制流边参数，传统 Phi 只保留等价语义对照；后续按 IR 表示、CFG/dominance、IR rewrite、标量 mem2reg、Machine IR 过渡层的顺序推进。
