...
- 后端承接 block arguments、Machine IR、栈帧、汇编输出和寄存器分配不再作为 A11 子任务，统一放入 A13。
...
### A13 RISC-V Machine IR 和寄存器分配优化

目标：在当前栈式后端稳定的基础上，引入一层轻量 Machine IR，把“指令选择、机器级控制流、栈帧布局、汇编打印、未来寄存器分配”拆成可独立演进的阶段。A13 的第一阶段不追求完整寄存器分配和性能优化，而是先完成可运行的 Machine IR MVP。

状态：第一阶段已完成。当前新增 `MachineFunction` / `MachineBasicBlock` / `MachineInstr` / `MachineOperand` / `MachineFrame`，`InstructionSelector` 将 Rewind IR 函数体 lowering 到 stack-backed Machine IR，`MachineVerifier` 和 `MachineAsmPrinter` 分别负责结构检查和最终 RISC-V/GAS 文本输出。默认 `-riscv` 已接入 Machine IR 路径；当前仍保留栈槽作为长期值来源，寄存器分配、peephole、block placement 和 instruction scheduling 放到后续阶段。

第一阶段目标：

- A13.1 定义 Machine IR core：`MachineFunction`、`MachineBasicBlock`、`MachineInstr`、`MachineOperand`、machine opcode、virtual register、frame index、block label 等基础结构。
- A13.2 实现 `InstructionSelector` 的平铺和映射：把 Rewind IR 指令线性 lowering 成带虚拟寄存器或 frame index 的 RISC-V Machine IR；第一阶段只做直接映射，不做 SelectionDAG / GlobalISel 级别复杂模式匹配。
- A13.3 承接 SSA block arguments：用 edge-copy / parallel-copy 语义表示 `jump %merge(value)` 和 `br cond, %t(args), %f(args)`，避免 true/false edge incoming value 混线。
- A13.4 实现 Machine IR verifier / printer：先能检查和打印 Machine IR，便于 smoke/golden 对照。
- A13.5 实现 stack-backed frame layout：在没有真正寄存器分配前，为需要落地的 virtual register、局部对象、outgoing args、saved `ra` 分配栈槽，并保持 RISC-V 栈对齐。
- A13.6 实现 prologue / epilogue insertion：在函数入口和返回路径插入开辟/释放栈帧、保存/恢复 `ra` 等指令。
- A13.7 实现 `MachineAsmPrinter`：把 Machine IR 转成 `AsmWriter` 调用，最终输出 RISC-V/GAS 汇编文本。

明确暂缓：

- A13 后续阶段再做 liveness analysis、linear scan / graph coloring register allocation、spill cost 决策、peephole、block placement 和 instruction scheduling。
- 第一阶段可以继续使用 scratch physical registers 做临时 materialize，并把长期值保存在 stack home slot 中；这不是最终性能方案，而是为了先打通 Machine IR 到 RISC-V asm 的可运行链路。

验收：

- Machine IR MVP 能覆盖现有 smoke 中的标量算术、控制流、函数调用、数组地址计算和返回。
- 启用 Machine IR 路径后能输出可汇编的 RISC-V asm，且现有 `-riscv` 回归不倒退。
- Machine IR 文本和 verifier 能暴露指令选择、edge-copy、栈帧和 terminator 结构错误。
- 后续引入寄存器分配时，不需要重新推翻 Machine IR / AsmPrinter / AsmWriter 边界。
